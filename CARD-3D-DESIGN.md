...
- 服务节点现在由三层轻量几何组成：六边形倒角徽章、原 SVG 挤出图标和按交互显隐的悬浮光环；徽章与光环复用共享几何，不引入纹理或外部模型。
- 分类平台增加一条低细分轨道环和一个轨道节点，把分类与服务的空间关系表达为导航装置；轨道在低画质下仍保持单一基础材质。
...
服务节点采用实体模型层：每个入口由带倒角的主体、嵌入式识别细节和统一底座组成，保留原始 SVG 几何作为不可见审计与 2D 降级数据源。通行证、云服务、信任中心、抽奖和 ID 使用独立模型，其余入口使用统一实体卡片结构，确保在低质量模式下仍控制面数与材质数量。
